...
if [ ! -f ${TRAVIS_BUILD_DIR}/already_signed ]; then
    ...
    openssl aes-256-cbc -K "${encrypted_334f60f39e2d_key}" -iv "${encrypted_334f60f39e2d_iv}" -in "${TRAVIS_BUILD_DIR}/retest-gmbh-gpg.asc.enc" -out "${TRAVIS_BUILD_DIR}/retest-gmbh-gpg.asc" -d
