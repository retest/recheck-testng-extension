...
public class RecheckHookTest {
	...
	private static class DummyTest {
		...
		private RecheckLifecycle lifecycle;
	...
	private void before() {
		...
		testInstance.lifecycle = mock( RecheckLifecycle.class );
		testName = "testName";
	...
	public void administersLifecycle() throws Exception {
		final InOrder inOrder = inOrder( callBack, testInstance.lifecycle );

		final RecheckHook hook = new RecheckHook();

		hook.run( callBack, testResult );

		inOrder.verify( testInstance.lifecycle ).startTest( testName );
		inOrder.verify( callBack ).runTestMethod( testResult );
		inOrder.verify( testInstance.lifecycle ).capTest();
		inOrder.verify( testInstance.lifecycle ).cap();
	}
	...
	public void usesParametersInTestName() throws Exception {
		...
		verify( testInstance.lifecycle ).startTest( combinedName );
	}
	...
	public void handlesNoParametersInTestName() throws Exception {
		...
		verify( testInstance.lifecycle ).startTest( testName );
	}
	...
	public void callCapAfterTestFailed() throws Exception {
		...
		doThrow( assertionError ).when( testInstance.lifecycle ).capTest();

		final RecheckHook hook = new RecheckHook();

		assertThatCode( () -> hook.run( callBack, testResult ) ).isEqualTo( assertionError );

		verify( testInstance.lifecycle ).cap();
	}
}
